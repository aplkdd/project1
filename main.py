...
Window.clearcolor = (.48, 0.27, 0.2, 0.4)
...
class CheckSits(Screen):
    ...
    def next(self):
        if not self.next_screen:
            ...
            gif_im = Image(source='Prised.gif', anim_delay=0.15)
            gif_im = Image(source='Prised.gif', anim_delay=0.15, size=(0.1, 0.1))
            gif_im.pos = (170, 570)
    ...
    def sec_finished(self, *args):
        if self.lbl_sec.done == True:
            if self.stage == 0:
                ...
                self.nxt_screen = True

    def next(self):
        if not self.nxt_screen:
            self.btn.set_disabled(True)
            self.lbl_sec.start()
        else:
            global p2, p3
